...
# Config
# =========================================================
CSV_PATH = Path(r"D:\audio_adv_experiment\datasets\torgo_single_word_headmic_split.csv")
#USE_SPLIT = "dev"   # 先跑 dev，之后改成 test
USE_SPLIT = None
OUT_PATH = Path(fr"D:\audio_adv_experiment\datasets\torgo_fulllex_wrong_subset_multi_hubert_{USE_SPLIT}_results.csv")
...
def normalize_waveform_for_model(waveform_1d: torch.Tensor) -> torch.Tensor:
...
def compute_ctc_loss(waveform_1d: torch.Tensor, target_text: str) -> torch.Tensor:
    x = normalize_waveform_for_model(waveform_1d)
...
print("\n=== HuBERT full-lexicon wrong-subset multi summary ===")
print(summary)

print("\n=== HuBERT full-lexicon rank change summary ===")
print(rank_change_summary)

print("\n=== HuBERT full-lexicon distance change summary ===")
...
oracle_out = OUT_PATH.with_name(OUT_PATH.stem + "_oracle_summary.csv")
oracle_df.to_csv(oracle_out, index=False, encoding="utf-8-sig")

print("\n=== HuBERT oracle / dynamic upper-bound summary ===")
...
print("\n=== HuBERT oracle improvement counts ===")
...
print("\nSaved oracle summary to:", oracle_out)