...
# Config
# =========================================================
CSV_PATH = Path(r"D:\audio_adv_experiment\datasets\torgo_10word_split_clean.csv")
OUT_PATH = Path(r"D:\audio_adv_experiment\datasets\torgo_10word_wrong_subset_multi_results.csv")
...
# =========================================================
# First pass: identify baseline wrong subset
# =========================================================
baseline_rows = []
...
# =========================================================
# Second pass: PGD only on wrong-subset
# =========================================================
results = []
...
for idx, row in wrong_df.iterrows():
    ...
    try:
        ...
        results.append({
            ...
            "utt_id": row["utt_id"],
            "target_word": target,
            "condition": "baseline_wrong",
            "epsilon": 0.0,
            "steps": 0,
            "raw_prediction": baseline_raw,
            "mapped_prediction": baseline_mapped,
            "mapped_correct": 0,
            "target_rank": baseline_target_rank,
            "top3_hit": int(baseline_target_rank <= 3),
            "top5_hit": int(baseline_target_rank <= 5),
            "top10_hit": int(baseline_target_rank <= 10),
            "target_distance": baseline_target_distance,
        ...
        print(
            f"[{idx}] target={target} | base_raw={baseline_raw} | "
            f"base_map={baseline_mapped} | base_rank={baseline_target_rank}"
        )
...
# Summary
# =========================================================
pgd_df = result_df[result_df["condition"] == "pgd_wrong_subset"].copy()

if len(pgd_df) == 0:
    print("No PGD wrong-subset results.")
    raise SystemExit(0)

pgd_df["rank_change"] = pgd_df["target_rank"] - pgd_df["baseline_target_rank"]
pgd_df["distance_change"] = pgd_df["target_distance"] - pgd_df["baseline_target_distance"]
pgd_df["repaired"] = (pgd_df["mapped_correct"] == 1).astype(int)
...
pgd_df["rank_change_label"] = pgd_df["rank_change"].apply(change_label)
pgd_df["distance_change_label"] = pgd_df["distance_change"].apply(change_label)

summary = (
    pgd_df.groupby(["epsilon", "steps"])
          .agg(
...
rank_change_summary = (
    pgd_df.groupby(["epsilon", "steps", "rank_change_label"])
          .size()
...
distance_change_summary = (
    pgd_df.groupby(["epsilon", "steps", "distance_change_label"])
          .size()
          .reset_index(name="count")
          .sort_values(["epsilon", "steps", "distance_change_label"])
)

print("\n=== 10-word wrong-subset multi summary ===")
print(summary)

print("\n=== 10-word rank change summary ===")
print(rank_change_summary)

print("\n=== 10-word distance change summary ===")
